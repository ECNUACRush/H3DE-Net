...
# from models.BiNet import HYATTNet
...
def test(dataloader, net, args):
    ...
    # Lists to store filenames and mean MREs
    filenames_list = []
    mre_list = []
    ...
    with torch.no_grad():
        for i, sample in enumerate(dataloader):
            ...
            filename = sample['filename'][0]  # Assuming batch size of 1, take the first filename

            # Send data to device
            data = data.to(DEVICE)

            # Get the proposal map from the network
            proposal_map = net(data)

            # Calculate MRE and hits, save the results in .npy format
            output_file = f"./hz_yolo_all/{filename}"  # Path to save .npy file
            ...
            # Update total hits and MREs
            total_hits += hits
            total_mre.append(np.array(mre))
            N += data.shape[0]

            # Calculate mean MRE for the current sample
            cur_mre = []
            for cdx in range(len(mre[0])):
                if mre[0][cdx] > 0:
                    cur_mre.append(mre[0][cdx])
            mean_mre = np.mean(cur_mre)
            total_mean_mre.append(mean_mre)

            # Save the filename and mean MRE for this sample
            filenames_list.append(filename)
            mre_list.append(mean_mre)

            print(f"#: No. {i}, Filename: {filename}, Current Mean MRE: {mean_mre:.4f}")

    # Concatenate all MREs for processing
    ...
    # Save per-landmark MRE to Excel
    ...
    # Save filenames and mean MREs for all samples
    results_df = pd.DataFrame({
        "Filename": filenames_list,
        "Mean MRE": mre_list
    })
    results_df.to_excel("yolo_results_summary.xlsx", index=False, header=True)
