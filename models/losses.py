...
class HNM_heatmap(nn.Module):
    ...
    def forward(self, heatmap, target_heatmap):
        loss = 0
        batch_size, n_class = heatmap.size(0), heatmap.size(1)
        ...
        for i in range(batch_size):
            for j in range(n_class):
                ...
                select_number = torch.sum(target_heatmap[i, j] >= 0).int().item()
                select_number = max(select_number, 1)  # Ensure at least 1 voxel

                # Limit select_number to avoid out-of-range errors
                select_number = min(select_number, heatmap[i, j].size(0))
                
                # Positive samples
                _, cur_idx = torch.topk(target_heatmap[i, j], select_number)
                assert cur_idx.max() < heatmap[i, j].size(0), "cur_idx out of range"
                ...
                neg_number = torch.sum(target_heatmap[i, j] < 0).int().item()
                neg_number = min(neg_number, heatmap[i, j].size(0))
                _, neg_idx = torch.topk(mask_neg, neg_number)
                assert neg_idx.max() < heatmap[i, j].size(0), "neg_idx out of range"
                predict_neg = heatmap[i, j].index_select(0, neg_idx)
                _, cur_idx = torch.topk(predict_neg, select_number)
